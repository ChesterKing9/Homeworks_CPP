...
public:
	HolySword() : Weapon("Holy Sword", 6) {}
	int getPower(Tribe monsterTribeModifier) const override {
		return monsterTribeModifier == Tribe::Zombie ? INT_MAX : m_power;
	}
	std::string getFullInfo() const override {
		return getName() + ", power: " + std::to_string(m_power) + ", instantly kills Zombies!";
	}
};
