...
void Munchkin::removeBiggestBonusItem() {
    if (m_items.empty()) {
        return;
    }

    auto maxIt = std::max_element(m_items.begin(), m_items.end(),
        [](Item* a, Item* b) { return a->getBasePower() < b->getBasePower(); });

    if (maxIt != m_items.end()) {
        m_items.erase(maxIt);
    }
}
